...
def add_lhcb_like_label(label=None, exp="LHCb", **kwargs):
    config = Config()
    plt.rcParams.update({"font.size": config.plots_mplhep_fontsize})
    ...
    kwargs.setdefault("fontsize", config.plots_mplhep_fontsize)
...
class ProfilePlotter(Plotter):
    ...
    def _plot_and_save_customizable_histogram(
        ...
        fig = plt.figure(
            figsize=(
                config.plots_figsize_x,
                config.plots_figsize_y * 1.5,
            ),
        )
        ...
        full_name = None
        if observable_name == "LatProf":
            full_name = "Lateral Profile"
        elif observable_name == "LongProf":
            full_name = "Longitudinal Profile"
        elif observable_name == "LatFirstMoment":
            full_name = "Lateral First Moment"
        elif observable_name == "LongFirstMoment":
            full_name = "Longitudinal First Moment"
        elif observable_name == "LatSecondMoment":
            full_name = "Lateral Second Moment"
        elif observable_name == "LongSecondMoment":
            full_name = "Longitudinal Second Moment"

        rlabel = (
            self._rlabel + "\n" + rf"$\bf{{{full_name.replace(' ', ';')}}}$"
        )
        ...
        if plot_profile:
            ...
            axes[0].step(
                ...
                full_simulation,
                label="FullSim",
                color=config.plots_full_sim_histogram_color,
            )
            axes[0].step(
                bins[:-1],
                ml_simulation,
                label="MLSim",
                color=config.plots_ml_sim_histogram_color,
            )
            axes[0].vlines(
                x=bins[0],
                ymin=0,
            ...
            energy_full_sim, _, _ = axes[0].hist(
                ...
                bins=bins,
                label="FullSim",
                histtype=config.plots_histogram_type,
                color=config.plots_full_sim_histogram_color,
            )
        ...
        if self._plot_gaussian:
            if plot_profile:
                (xs_full_sim, ys_full_sim) = _best_fit(
                    full_simulation, bins, hist=True
                )
                (xs_ml_sim, ys_ml_sim) = _best_fit(
                    ml_simulation, bins, hist=True
                )
            else:
            ...
            axes[0].plot(
                xs_full_sim,
                ys_full_sim,
                color=config.plots_full_sim_gaussian_color,
                label="FullSim",
            )
            axes[0].plot(
                xs_ml_sim,
                ys_ml_sim,
                color=config.plots_ml_sim_gaussian_color,
                label="MLSim",
            )

        if y_log_scale:
            axes[0].set_yscale("log")
        axes[0].legend(loc=config.plots_mplhep_legend_loc)
        ...
        add_lhcb_like_label(
            ax=axes[0],
            exp=config.plots_mplhep_experiment_header_name,
            llabel=config.plots_mplhep_llabel_header_name,
            rlabel=rlabel,
            loc=config.plots_mplhep_legend_loc,
            fontsize=config.plots_mplhep_fontsize,
        )
        axes[0].set_ylim(axes[0].get_ylim()[0], axes[0].get_ylim()[-1] * 1.3)
        ...
        # )
        fig.canvas.draw()
        plt.savefig(f"{config.output_area}/{observable_name}.png")
        plt.clf()
    ...
    def _plot_profile(self) -> None:
        ...
        if self._profile_type == ProfileType.LONGITUDINAL:
            ...
            bins = np.linspace(
                0, config.cylinder_z_cell_no, config.cylinder_z_cell_no + 1
            )
            observable_name = "LongProf"
            xlabel = "Layer index"
        else:
            bins = np.linspace(
                0, config.cylinder_rho_cell_no, config.cylinder_rho_cell_no + 1
            )
    ...
    def _plot_first_moment(self) -> None:
        ...
        full_simulation_first_moment = (
            self._full_simulation.calc_first_moment()
        )
        ...
        if self._profile_type == ProfileType.LONGITUDINAL:
            ...
            bins = np.linspace(
                0,
                0.4 * config.cylinder_z_cell_no * config.cylinder_z_cell_size,
                128,
            )
        else:
            xlabel = "$<r> [mm]$"
    ...
    def _plot_second_moment(self) -> None:
        ...
        full_simulation_second_moment = (
            self._full_simulation.calc_second_moment()
        )
        ...
        if self._profile_type == ProfileType.LONGITUDINAL:
            ...
            bins = np.linspace(
                0,
                pow(config.cylinder_z_cell_no * config.cylinder_z_cell_size, 2)
                / 35.0,
                128,
            )
        else:
    ...
    def plot_and_save(self) -> None:
        ...
        config.log.debug(f"Plotting the {profile_type_name} profile...")
        self._plot_profile()
        config.log.debug(
            f"Plotting the first moment of {profile_type_name} profile..."
        )
        self._plot_first_moment()
        config.log.debug(
            f"Plotting the second moment of {profile_type_name} profile..."
        )
...
class EnergyPlotter(Plotter):
    ...
    def _plot_total_energy(self, y_log_scale=True) -> None:
        ...
        """
        config = Config()
        full_simulation_total_energy = (
            self._full_simulation.calc_total_energy()
        )
        ...
        plt.legend(loc=config.plots_mplhep_legend_loc)
        if y_log_scale:
            plt.yscale("log")
        ...
        #     f" $e^-$, {self._particle_energy} [GeV], {self._particle_angle}$^{{\circ}}$, {self._geometry} "
        # )
        plt.savefig(f"{config.output_area}/E_tot.png")
        plt.clf()
    ...
    def _plot_cell_energy(self) -> None:
        ...
        ax.set_ylim(ax.get_ylim()[0], ax.get_ylim()[-1] * 1.3)
        fig.canvas.draw()
        plt.savefig(f"{config.output_area}/E_cell.png")
        plt.clf()

    def _plot_energy_per_layer(self):
        """Plots and saves config.cylinder_z_cell_no histograms with total energy detected in particular layers.
        ...
        """
        config = Config()
        full_simulation_energy_per_layer = (
            self._full_simulation.calc_energy_per_layer()
        )
        ml_simulation_energy_per_layer = (
            self._ml_simulation.calc_energy_per_layer()
        )
        ...
        fig, ax = plt.subplots(
            ...
            figsize=(
                config.plots_figsize_x * number_of_plots_in_column / 3,
                config.plots_figsize_y * number_of_plots_in_row / 3,
            ),
            sharex="all",
            sharey="all",
        ...
        for layer_nb in range(config.cylinder_z_cell_no):
        ...
        plt.savefig(
            f"{config.output_area}/E_layer.png",
            bbox_inches="tight",
        )
        plt.clf()
    ...
    def plot_and_save(self):
        ...
        """
        config = Config()
        config.log.debug("Plotting total energy...")
        self._plot_total_energy()
        config.log.debug("Plotting cell energy...")
